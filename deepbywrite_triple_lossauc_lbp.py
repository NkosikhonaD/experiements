# -*- coding: utf-8 -*-
"""DeepByWrite_triple_lossAUC_LBP.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1upBaXN5beqxa-kh79zZhzZYcNar6dsdd
"""
...
class TripletDataset(Dataset):
  ...
  def __getitem__(self,idx):
    ...
    anc_img=cv2.imread(anc_img,0) 
    pos_img=cv2.imread(pos_img,0)
    neg_img=cv2.imread(neg_img,0)
    
    anc_img_lbp= feature.local_binary_pattern(anc_img,24,8,method="uniform")
    #print(anc_img_lbp)
    #(hist_anc,_) =np.histogram(anc_img_lbp.ravel(),bins=np.arange(0,27),range=(0,26))
    #hist_anc = hist_anc.astype("float")
    #hist_anc /=(hist_anc.sum()+eps)
    
    pos_img_lbp= feature.local_binary_pattern(pos_img,24,8,method="uniform")
    #(hist_pos,_) =np.histogram(pos_img_lbp.ravel(),bins=np.arange(0,27),range=(0,26))
    #hist_pos = hist_pos.astype("float")
    #hist_pos /=(hist_pos.sum()+eps)
    
    
    neg_img_lbp= feature.local_binary_pattern(neg_img,24,8,method="uniform")
    
    (hist_neg,_) =np.histogram(neg_img_lbp.ravel(),bins=np.arange(0,27),range=(0,26))
    #hist_neg = hist_neg.astype("float")
    #hist_neg /=(hist_neg.sum()+eps)

    pos_class=torch.from_numpy(np.array([pos_class]).astype('long'))
    neg_class=torch.from_numpy(np.array([neg_class]).astype('long'))

    sample = {'anc_img':anc_img_lbp,'pos_img':pos_img_lbp,'neg_img':neg_img_lbp,'pos_class':pos_class,'neg_class':neg_class}
...
plt.savefig('/content/drive/My Drive/ColabNotebooks/log/lossLPB.jpg', dpi = fig.dpi)
...
for phase in ['train', 'valid']:
  ...
  if phase == 'train':
    plt.plot(list_epoch, list_accuracy, color = 'red', label = 'Validate: Set')
  else:
    plt.plot(list_epoch, list_accuracy, color = 'blue', label = 'Test: Set')
...
plt.legend(loc='upper right')
plt.savefig('/content/drive/My Drive/ColabNotebooks/log/accuracyLBP.jpg', dpi = fig2.dpi)
