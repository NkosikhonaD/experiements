# -*- coding: utf-8 -*-
"""DeepByWrite_triple_lossAUC_HOG.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ZLj_kNV0LwGACPpArljfK4JFmMfIXU9b
"""
...
# youtube tutorials : 3BLUE1BROWN SERIES
...
class TripletDataset(Dataset):
  ...
  def __getitem__(self,idx):
    ...
    (anc_img_hog,hogImagea)= feature.hog(anc_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
    (pos_img_hog,hogImagep)= feature.hog(pos_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
    (neg_img_hog,hogImagen)= feature.hog(neg_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
    

    pos_class=torch.from_numpy(np.array([pos_class]).astype('long'))
    neg_class=torch.from_numpy(np.array([neg_class]).astype('long'))

    sample = {'anc_img':hogImagea,'pos_img':hogImagep,'neg_img':hogImagen,'pos_class':pos_class,'neg_class':neg_class}
...
class LetterNetModel(nn.Module):
  def __init__(self,embedding_size,num_classes):
    ...
    #self.flat_size=  
    self.fc1 = nn.Linear(128*128,640) #get size of HOG features is the in put dimesion of HOG check output shape 
    self.fc2 = nn.Linear(640,200) # input dimmesion for the fc1 
    self.fc3 = nn.Linear(200,embedding_size) # out put length of Embedding ? see later 
...
def train_valid(model, optimizer, scheduler, epoch, dataloaders, data_size):
  ...
  for phase in ['train', 'valid']:
    ...
    for batch_idx, batch_sample in enumerate(dataloaders[phase]):
      #This is where we need to convert the images into hog features, but the data loaders 
      #already converted tem to tensors .. removed this and made them PL images in the dataloaders...
      ...
      #anc_img = img_trans(anc_img) # convert tensors to PILImages
      #(anc_img_hog,hogImage)= feature.hog(anc_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
      #(pos_img_hog,hogImage)= feature.hog(pos_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
      #(neg_img_hog,hogImage)= feature.hog(neg_img,orientations=9,pixels_per_cell=(10,10),cells_per_block=(2,2),transform_sqrt=True,feature_vector=True,block_norm="L1",visualize=True)
      #pos_img = img_trans(pos_img)
      #neg_img = img_trans(neg_img)
      #print(anc_img)
      #anc_img.show()
      ...
      #anc_img_hog = torch.from_numpy(anc_img_hog).to(device)
      #pos_img_hog = torch.from_numpy(pos_img_hog).to(device)
      #neg_img_hog = torch.from_numpy(neg_img_hog).to(device)
      
      with torch.set_grad_enabled(phase == 'train'):
        # anc_embed, pos_embed and neg_embed are encoding(embedding) of image
        anc_embed, pos_embed, neg_embed = model(anc_img.double()), model(pos_img.double()), model(neg_img.double())
    ...
    #print('  {} set - Precision           = {:.8f}'.format(phase, np.mean(pres)))
    if phase =='valid':
      model_accuracy.append(np.mean(accuracy))
      model_precision.append(np.mean(pres))
...
train_path = "/content/drive/My Drive/ColabNotebooks/DataProcessed/final_data_80_20/training_5/"
test_path = "/content/drive/My Drive/ColabNotebooks/DataProcessed/final_data_80_20/testing_5/"
...
print("Accuracy all epochs ",str(np.mean(model_accuracy)))
print("Mean precision all epochs ",str(np.mean(model_precision)))
...
for phase in ['train', 'valid']:
  ...
  if phase == 'train':
    plt.plot(list_epoch, list_loss, color = 'red',label = 'Validate: Set')
  else:
    plt.plot(list_epoch, list_loss, color = 'blue',label = 'Test: Set')
  plt.xlabel('Epoch', fontsize = 15)
  plt.ylabel('Loss', fontsize = 15)
  plt.ylim(0, 1)
plt.legend(loc='upper right')   
plt.savefig('/content/drive/My Drive/ColabNotebooks/log/lossHOG.jpg', dpi = fig.dpi)
...
plt.legend(loc='upper right')
plt.savefig('/content/drive/My Drive/ColabNotebooks/log/accuracy.jpg', dpi = fig2.dpi)

print(" mean average accuracy ",np.mean(model_accuracy))
#print("mean average precision ",np.mean(model_precision)) 

#print("max accuracy ",np.max(model_accuracy))
#print ("max precision ",np.max(model_precision))